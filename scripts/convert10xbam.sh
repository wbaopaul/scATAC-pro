...
cat <( ${mapRes_dir}/${bamName}.header )\
<( ${SAMTOOLS_PATH}/samtools view $input_bam | awk '{for (i=12; i<=NF; ++i) { if ($i ~ "^CB:Z:"){ td[substr($i,1,2)] = substr($i,6,length($i)-5); } }; printf "%s:%s\n", td["CB"], $0 }' ) \
| ${SAMTOOLS_PATH}/samtools view -bS - > ${mapRes_dir}/${OUTPUT_PREFIX}.positionsort.bam

samtools view ${mapRes_dir}/${OUTPUT_PREFIX}.positionsort.bam | cut -f 1 | head 
