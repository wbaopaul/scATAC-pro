...
### Clustering with different paramters
```{r}
if (file.exists(seurat_file)) {
  shinyApp(
  ui <- fillPage(fillCol(flex = c(NA, 1),
        inputPanel(
          textInput("dims", "dims (reduced dimensions):", '1:30'),
          numericInput("resolution", "resolution (of clustering):", 0.2),
  ...
  server <- function(input, output) {
    observeEvent(input$update, {
      range_values <- strsplit(input$dims, ":")[[1]]
      pc_start <- as.integer(range_values[1])
      pc_end <- as.integer(range_values[2])
      ss = FindNeighbors(ss, dims = seq(pc_start, pc_end), verbose = F)
      ss = FindClusters(ss, resolution = input$resolution, verbose = F)
      output$umapPlot <- renderPlot({
       DimPlot(ss, reduction = 'umap') + scale_color_brewer(palette = 'Paired')
    })
  })
 }
)
...
if (file.exists(seurat_file)) {
  ...
  if (grepl(GENOME_NAME, pattern = 'mm10', ignore.case = TRUE)) {
    library(EnsDb.Mmusculus.v79)
    ens.ann <- EnsDb.Mmusculus.v79
  }
  
  if (grepl(GENOME_NAME, pattern = 'mm9', ignore.case = TRUE)) {
    library(EnsDb.Mmusculus.v75)
    ens.ann <- EnsDb.Mmusculus.v75
  }
  
  if (grepl(GENOME_NAME, pattern = 'hg38', ignore.case = TRUE)) {
    library(EnsDb.Hsapiens.v86)
    ens.ann <- EnsDb.Hsapiens.v86
  }
  
  if (grepl(GENOME_NAME, pattern = 'hg19', ignore.case = TRUE)) {
    library(EnsDb.Hsapiens.v75)
    ens.ann <- EnsDb.Hsapiens.v75
  }
  
  annotations <- GetGRangesFromEnsDb(ensdb = ens.ann)
  ...
  if (class(ss[['ATAC']]) == 'Assay5') {
    atac_counts = ss[['ATAC']]$counts
  } else {
    atac_counts = ss[['ATAC']]@counts
  }
  ...
  ss[['peaks']] <- chrom_assay
  DefaultAssay(ss) <- 'peaks'
  ss[['ATAC']] <- NULL # save space
...
shinyApp(
  ui <- fillPage(fillCol(flex = c(NA, 1),
        inputPanel(
          textInput("region", "Region or gene Name:", "chr1-50000000-50010000"),
          numericInput("up_dist", "Upstream of the region (kb):", 10),
          numericInput("down_dist", "Downstream of the region (kb):", 10),
          actionButton("update", "Update View")
        ),
        plotOutput("covPlot", height = "100%")
      )
),
  server <- function(input, output) {
    observeEvent(input$update, {
      tss_ann <- Signac:::CollapseToLongestTranscript(annotations)
      if(input$region %in% tss_ann$gene_name){
        hl.region = tss_ann[tss_ann$gene_name == input$region, ]
      }else{
        hl.region = StringToGRanges(input$region)
      }
      output$covPlot <- renderPlot({
       CoveragePlot(
        ...
        assay = 'peaks',
        region = input$region,
        region.highlight = hl.region,
        group.by = 'active_clusters',
        peaks = TRUE,
        extend.upstream = input$up_dist * 1000,
        extend.downstream = input$down_dist * 1000
      )
    })
  })
 }
