...
library(ggplot2)
library(grid)
g <- ggplot(data = bc_stat[sample(1:nrow(bc_stat), 15000), ], 
...
if(file.exists(go_file)){
   #library(xlsx)
    #go_res = read.xlsx(go_file, sheetName = paste0('cluster', group1))
    ...
    p_go <- ggplot(go_res, aes(y = score, x = Description, fill = Count)) +
      ...
      ggtitle(paste0("Enriched terms: cluster", group1)) + theme_classic() + 
...
### Predicted Interactions at CD14 locus
```{r, out.width='\\textwidth', message=F}
...
if(file.exists(cicero_conn.file)){
 ...
 if(plotEPS){
   postscript(paste0(params$output_dir,'/summary/Figures/interactions_CD14_locus.eps'), width = 8, height = 5, fonts = 'sans')
    plot_connections(conns, "chr5", 140610000, 140640000,
