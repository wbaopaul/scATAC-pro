...
cell.table = data.frame(c(CELL_CALLER,  paste0(ncells), nrow(peaks), paste0(med.frag.per.cell), frac.in.cell, mapq30.frac.in.cell)) 

rm(peaks)

rownames(cell.table) = c('Cell called by', 'Estimated # of cells', 'Number of peaks',  'Median fragments per cell', 'Fraction of Mapped reads in cells', 'Fraction of MAPQ30 in cells')  
kable(cell.table, row.names = T, col.names = NULL, format = 'html') %>%
  kable_styling("striped", full_width = F, position = 'left', font_size = 15)

write.table(cell.table, file = paste0(params$output_dir, '/summary/Tables/Cell_Calling_Summary.tsv'),
...
if(file.exists(go_file)){
    ...
    if(nrow(go_res) == 0) {
        if(grepl(group1, pattern = 'one')) {
            print(paste0('No enriched terms in comparison of ', group1, '_vs_', group2))}else{
            print(paste0('No enriched terms in comparison of cluster_', group1, '_vs_', group2))
          }
    }else{
...
if(file.exists(paste0(down.dir, '/chromVar_obj.rds'))){
  ...
  if(!file.exists(paste0(down.dir, '/differential_TF_motif_enriched_in_clusters.txt'))){
    ...
    da.res = fread( paste0(down.dir, '/differential_TF_motif_enriched_in_clusters.txt'))
  }
