...
if(file.exists(paste0(down.dir, '/chromVar_obj.rds'))){
  ...
  write.csv(da.res, file = paste0(down.dir, '/differential_TF_accessibility.csv'), quote = F, row.names = F )
...
comp_cls = paste0('cluster_', cluster1, '_cluster_', cluster2)
...
#  library(grid)
  ...
  footprint_stats[, 'isSig' := ifelse(P_values <= 0.05 & TF_Activity > 0, paste0('cluster', cluster2, '_high'), isSig)]
   footprint_stats[, 'isSig' := ifelse(P_values <= 0.05 & TF_Activity < 0, paste0('cluster', cluster1, '_high'), isSig)]
  footprint_stats$motif_show = ""
  footprint_stats[P_values <= 0.05]$motif_show = footprint_stats[P_values <= 0.05]$motif
  
  
  
 p <- ggplot(data = footprint_stats, aes(x = motif, y = TF_Activity, colour = factor(isSig), label = motif_show)) + geom_point() + xlab("") + ylab('TF Activity Difference') + theme(legend.text = element_text(size=15), axis.title = element_text(size = 15), axis.text.x = element_blank(), legend.title = element_blank(), legend.direction = 'horizontal', plot.title = element_text(size = 15, face = 'bold')) + ggtitle(paste0('TF Activity difference: cluster', cluster2, '-', 'cluster', cluster1  )) + geom_text_repel(force = 10) + theme(plot.margin=unit(c(0.2, 0, 0.2, 0), "cm"), legend.position = 'bottom') + scale_color_manual(values = c('#F8766D', '#619CFF', 'gray40'))
...
if(file.exists(footprint_stats.file)){
 ...
 dd[, 'id' := -99:100]
 dd1 = rbind(subset(dd, select = c('id', paste0('cluster_', cluster1))))
 dd2 = rbind(subset(dd, select = c('id', paste0('cluster_', cluster2))))
 names(dd1)[2] = names(dd2)[2] = 'score'
 dd = rbind(dd2, dd1)
 dd$cluster = rep(c(paste0('cluster_', cluster2), paste0('cluster_', cluster1)), each = nrow(dd1))
