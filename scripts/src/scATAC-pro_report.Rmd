...
plotPDF = as.logical(plotPDF)
if(plotPDF) dir.create(paste0(params$output_dir, '/summary/Figures'), showWarn = F)
...
if(plotPDF){
  pfname = paste0(params$output_dir, '/summary/Figures/dist_insert_size.pdf')
  ggsave(p1, file = pfname, device = 'pdf', width = 6, height = 6)
}
rm(frag_lens)
invisible(gc())
...
if(plotPDF){
  ...
  pfname = paste0(params$output_dir, '/summary/Figures/dist_frags.pdf')
  ggsave(p, file = pfname, device = 'pdf', width = 6, height = 6)
}
rm(bc_stat)
invisible(gc())
...
if(file.exists(tss_escore_file)){
    ...
    if(plotPDF){
      ...
      pfname = paste0(params$output_dir, '/summary/Figures/tss_enrich.pdf')
      ggsave(p0, file = pfname, device = 'pdf', width = 6, height = 6)
    }
}
...
if(plotPDF){
  pfname = paste0(params$output_dir, '/summary/Figures/overlap_annotation.pdf')
  ggsave(p0, file = pfname, device = 'pdf', width = 6, height = 6)
}
...
if(file.exists(seurat_file)){
    ...
    if(plotPDF){
      pfname = paste0(params$output_dir, '/summary/Figures/umap_clusters.pdf')
...
if(file.exists(go_file)){
    ...
    if(nrow(go_res) == 0) {
        ...
        if(plotPDF){
            pfname = paste0(params$output_dir, '/summary/Figures/enriched_GO.pdf')
            ggsave(p_go, file = pfname, device = 'pdf', width = 10, height = 6)
        }
    }
}else{
...
if(file.exists(paste0(down.dir, '/chromVar_obj.rds'))){
  ...
  if(plotPDF){
  pfname = paste0(params$output_dir, '/summary/Figures/heatmap_motif_enrich.pdf')
...
if(file.exists(footprint_stats.file)){
  ...
  if(plotPDF){
    ggsave(p1, filename = pfname1, device = 'pdf', height = 6,
           width = 6)
  }
