...
if [ ! $USE_BIN ]; then 
    ...
    peak_dir=${mat_dir}/peak_mat
    mkdir -p $peak_dir
    ${R_PATH}/R --vanilla --args ${mat_dir}/fragments.bed $feature_file ${peak_dir}/${OUTPUT_PREFIX}.${MAPPING_METHOD}.peak.barcode.mtx < ${curr_dir}/src/get_mtx.R 
fi
