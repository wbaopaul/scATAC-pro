...
writeMM(out_mat, file = paste0(output_pre, 'filtered.fdr', fdr, '.mtx'))  
write.table(barcodes, file = paste0(output_pre, 'filtered.fdr', fdr, '.barcodes.txt'), sep = '\t', row.names = F, quote = F, col.names = F)
write.table(features, file = paste0(output_pre, 'features.txt'), sep = '\t', row.names = F, quote = F, col.names = F)
