...
```
    ...
    $ scATAC-pro -s rmDoublets
                 -i output/filtered_matrix/PEAK_CALLER/CELL_CALLER/seurat_obj.rds,0.03 (0.03 is the expected fraction of doublets ) 
                 -c configure_user.txt
    ...
    OPTIONS
       ...
       [-s|--step ANALYSIS_STEP] : run an analysis module (or some combination of several modules) of the scATAC-pro workflow, supported modules include:
          ...
          rmDoublets: remove potential doublets
                         input: a seurat object outputted from clustering module
                         output: a seurat object with doublets removed saved in the save directory (and a umap colored by singlet/doubet) 
